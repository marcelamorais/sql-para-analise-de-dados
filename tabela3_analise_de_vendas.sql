...
Arquivo: tabela3_analise_de_vendas.sql
...
de negócio como faturamento, ranking de clientes e
...
- CASE WHEN para classificação de resultados
...
-- Cria o banco de dados tabela3
CREATE DATABASE tabela3;
...
SELECT
    orderdate,
    COUNT(orderid) AS numerodepedidos
FROM orders
GROUP BY orderdate;

-- Três clientes que mais gastaram
SELECT
    c.customername,
    SUM(o.total) AS valorgastototal
FROM customers c
JOIN orders o
    ON c.customerid = o.customerid
GROUP BY c.customername
...
-- View com o total de vendas por cliente
